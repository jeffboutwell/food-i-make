...
const HitsList = memo(function HitsList({
  ...
  const highlightAttribute = toAttributePath(mapping.primaryText);

  if (!attributes || !mapping.primaryText || !highlightAttribute) {
          ...
          >
            ...
            <div>
              <p className="font-medium [&_mark]:bg-transparent [&_mark]:text-secondary-foreground [&_mark]:underline [&_mark]:underline-offset-4">
                <Highlight attribute={highlightAttribute} hit={hit} />
              </p>
...
export default function SearchExperience(config: SearchConfig) {
  ...
  return (
    ...
    >
      <SearchButton {...config.buttonProps} onClick={openModal}>
